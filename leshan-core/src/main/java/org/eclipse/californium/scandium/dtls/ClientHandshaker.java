 ...
 * as delivered by the parent {@link Handshaker} class.
 */
public class ClientHandshaker extends Handshaker {
    ...
    /*
     * Store all the message which can possibly be sent by the server. We need these to compute the handshake hash.
     */
     ...
     * @param pskStore storage for the pre-shared-keys
     */
    public ClientHandshaker(InetSocketAddress endpointAddress, RawData message, DTLSSession session,
            Certificate[] rootCerts, ClientConnectorConfig config) {
    ...
    /**
     * Called when the client received the server's finished message. If the data can be verified, encrypted application
     * data can be sent.
     * 
     ...
     * 
     * @param message the hello request message
     */
    ...
    /**
     * A {@link HelloVerifyRequest} is sent by the server upon the arrival of the client's {@link ClientHello}. It is
     * sent by the server to prevent flooding of a client. The client answers with the same {@link ClientHello} as
     * before with the additional cookie.
     * 
    ...
    /**
     * Unless a anonymous cipher suite is used, the server always sends a {@link CertificateMessage}. The client
    ...
    /**
     * The ServerKeyExchange message is sent by the server only when the server {@link CertificateMessage} (if sent)
     * does not contain enough data to allow the client to exchange a premaster secret. Used when the key exchange is
     * ECDH. The client tries to verify the server's signature and on success prepares the ECDH key agreement.
     * 
     ...
     * The client prepares all necessary messages (depending on server's previous flight) and returns the next flight.
     * 
    ...
    private DTLSFlight receivedServerHelloDone(ServerHelloDone message) throws HandshakeException {
        ...
        /*
         * All possible handshake messages sent in this flight. Used to compute handshake hash.
         */
        ...
        /*
         * Second, send ClientKeyExchange as specified by the key exchange algorithm.
         */
        ...
        if (certificateRequest != null) {
            ...
            SignatureAndHashAlgorithm signatureAndHashAlgorithm = certificateRequest.getSupportedSignatureAlgorithms()
                    .get(0);
